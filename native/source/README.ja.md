...
初期のソーススナップショットは、デスクトップアプリの `db/duck.rs` からコピーされました。

ソースSHA-256: `c828ca26090f8699aa2a5e04cca0f328732469aa938b47ecd2bd198877c104c0`。


このディレクトリは `irodori.motherduck` のマイグレーションステージングエリアです。アクティブなネイティブのエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットから動作モジュールに移動すべきであり、コネクターのランタイム契約がデスクトップアプリに組み込まれています。
...
`knowledge/engines.json` からのエンジンステータス: `extension`。